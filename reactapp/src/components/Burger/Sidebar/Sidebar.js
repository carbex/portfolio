...
export function Sidebar(props) {
  // DEFAULT PROPS
  const { isSidebarOpen = true, isSidebarLarge = true } = props;
  ...
  // Update of header state and menuItemsUpdated state
  useEffect(() => {
    let timer;
    if (isSidebarLarge) {
      timer = setTimeout(() => {
        setHeader(sidebar.header.fullName);
        setMenuItemsUpdated(menuItems);
      }, 200);
    } else {
      setHeader(sidebar.header.shortName);
  ...
  }, [location.pathname, menuItemsUpdated]);
  ...
  const handleMenuItemClick = (id, index) => {
    if (id === 6) {
      dispatch(addUser({ login: null, token: null, role: null }));
      setMenuItems([
        ...sidebar.menuItems.map((item) => {
          if (item.to === "login") {
  ...
  const menuItemsJSX = menuItems.map((item, index) => {
    if (item.id === 4 && !token) {
      ...
      // const hasSubmenus = !!item.subMenuItems.length;
      const isOpen = subMenusStates[index]
        ? subMenusStates[index].isOpen
        : null;
        ...
        <S.ItemContainer key={index}>
          <Link to={item.to} style={{ textDecoration: "none" }}>
            ...
            >
              ...
              <S.Text isSidebarLarge={isSidebarLarge}>
                {menuItemsUpdated[index].name}
              </S.Text>
          ...
          {/* <AnimatePresence>
            {hasSubmenus && isOpen && (
              <motion.nav
                ...
                exit={{ opacity: 0, x: -30 }}
              >
                <S.SubMenuItemContainer isSidebarLarges={isSidebarLarge}>
                  {subMenuJSX}
