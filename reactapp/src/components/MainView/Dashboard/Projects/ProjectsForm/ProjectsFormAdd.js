...
export function ProjectFormAdd(props) {
  // Default props
  const { onSubmit, unSetModal } = props;
  ...
  return (
    <S.Container>
      ...
      <S.ImageContainer>
        <S.Image
          src={imageUrl ? imageUrl : "http://via.placeholder.com/700x525"}
          alt={title ? title : "project_image"}
        />
      </S.ImageContainer>
      <label htmlFor="image">
        Image* (.SVG, .TIFF, .PNG, .GIF, .JPGs, .WebPs)
      </label>
      ...
      <S.Input
        type="text"
        name="resources"
        placeholder="HTML|CSS|REACT|MONGOOSE"
