...
import { ProjectApiService } from "../../../../api-services/project";
...
export function Projects() {
  ...
  const loadProjects = useCallback(async () => {
    let rawResponse = await projectApiService.loadProjects();
  ...
  const handleAddProjectSubmit = async (
    ...
    const rawResponse = await projectApiService.addProject(data);
    let response = await rawResponse.json();
  ...
  const handleUpdateProjectSubmit = async (
    ...
    const rawResponse = await projectApiService.updateProject(data);
    let response = await rawResponse.json();
  ...
  const handleDeleteProjectSubmit = async (id, imageUrl) => {
    let publicId = imageUrl.split("/").pop().split(".").shift();
    const rawResponse = await projectApiService.deleteProject({ id, publicId });
    let response = await rawResponse.json();
    if (response.result) {
      loadProjects();
      unSetModal();
